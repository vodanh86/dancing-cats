...
inline void add_reflections(inout half3 fragment_color, const in float3 view_direction_ws, const in float3 normal_ws, const in float3 position_ws, const in float2 normalized_screen_space_uv, const in half3 albedo)
{
    ...
    reflection = CalculateIrradianceFromReflectionProbes(reflect_vector, position_ws, roughness, normalized_screen_space_uv);
    #else
